...
object PurchaseRepository {
    ...
    init {
        ...
        purchases.add(Purchase(10L, 1515L, 5L, 150.00, "2024-06-25", "11J"))
    }
    ...
    fun getByUserId(idUsuario: Long) : List<Purchase> {

        val result = purchases.filter { it.userId == idUsuario }
        if (result.isEmpty()) {
            println("No hay compras")
        }
        return result
    }
    ...
    fun estaOcupado(eventId: Long, asiento: String): Boolean {
        return purchases.any { it.eventId == eventId && it.seat == asiento }
    }
}